...
let root = website
#else
let root = "file://" + (__SOURCE_DIRECTORY__ @@ "../output/es")
#endif

// Paths with template/source/output locations
let bin        = __SOURCE_DIRECTORY__ @@ "../../src/FsCheck.Xunit/bin/Release" //might not work in the future
let content    = __SOURCE_DIRECTORY__ @@ "../content/es"
let output     = __SOURCE_DIRECTORY__ @@ "../output"
let outputEs   = __SOURCE_DIRECTORY__ @@ "../output/es"
let files      = __SOURCE_DIRECTORY__ @@ "../files"
let templates  = __SOURCE_DIRECTORY__ @@ "templates/es"
...
let buildDocumentation () =
  let subdirs = Directory.EnumerateDirectories(content, "*", SearchOption.AllDirectories)
                |> Seq.filter (fun x -> x.Contains "es")
  for dir in Seq.append [content] subdirs do
    let sub = if dir.Length > content.Length then dir.Substring(content.Length + 1) else "."
    Literate.ProcessDirectory
      ( dir, docTemplate, outputEs @@ sub, replacements = ("root", root)::info,
